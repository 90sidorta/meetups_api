...
from app.schemas.meetups import MeetupResponse, MeetupAddData


## Create ###
@pytest.mark.parametrize(
    "title, description, address, published, price, limit",
    [
...
def test_get_meetups_unauthorized(
    client, limit, skip, search, length, test_load_meetups
):
    url = (
...
def test_get_meetups(
    authorized_client, test_load_users, limit, skip, search, length, test_load_meetups
):
    url = (
        f"?limit={limit}&skip={skip}&search={search}"
        if search
...
def test_get_one_meetup_not_exists(
    authorized_client, test_load_users, test_load_meetups
):
...
# Update ###
def test_update_meetup_not_exists(
    authorized_client, test_load_users, test_load_meetups
):
...
def test_update_meetup_other_user(
    authorized_client, test_load_users, test_load_meetups
):
    res = authorized_client.put(
        f"/meetups/1",
...
## Delete ###
def test_delete_meetup_not_exists(
    authorized_client, test_load_users, test_load_meetups
):
...
def test_delete_meetup_other_user(
    authorized_client, test_load_users, test_load_meetups
):
