...
def test_create_user(client):
    res = client.post("/users/", json={"email": "test2@test.com", "username": "testowy", "password": "testtest2"})
...
def test_login_user(client, test_load_users, test_user):
    res = client.post("/login", data={"username": "test0@test.com", "password": "testtest2"})
    schema.AccessToken(**res.json())
